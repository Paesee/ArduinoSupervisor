...
extern "C"
{
  ...
  float GeneralPID_execute(GeneralPID *self, float measurement)
  {
    ...
    filtered_meas = 0.4665 * measurement + 0.5335 * self->last_measurement;

    // calcula do erro
    float error = self->setpoint - filtered_meas;
    ...
    if (is_saturated)
    {
      ...
      //output = self->second_last_output + self->K1 * error + self->K2 * self->last_error + self->K3 * self->second_last_error;
    }
    else
    {
      output = self->second_last_output + self->K1 * error + self->K2 * self->last_error + self->K3 * self->second_last_error;
    ...
    // armazena o valor atual de measurement
